...
{
    struct flight_alarm *fa = stats.fl_alarms;
    u8 i;
...
static void shell_cmd_alarms(char *args, void *data)
{
    ...
    u8 t, n = 0xff, id, action = 0, nr_alarms = 0;
    ...
    if ((t < 1) || (p == NULL)) {
        ...
        shell_printf("\nsyntax: flight alarms -a <action> [-n <nr>|-i <id>] -v <value> -t <timer>\n");
        shell_printf("      <action> 0=remove, 1=add low limit, 2=add high limit, 3=remove all\n");
    } else {
        action = atoi(p->val);
        for (nr_alarms = 0; nr_alarms < MAX_FLIGHT_ALARMS; nr_alarms++) {
            if (stats.fl_alarms[nr_alarms].cfg == NULL)
                break;
        }
        switch (action) {
            default:
            case 0:
                ...
                if (n >= nr_alarms) {
                    shell_printf("Invalid alarm nr specified...\n");
                    return;
                }
                ...
                shell_printf("Removing %s alarm nr %u (%u total)\n",
                        fa[n].info->name, n, nr_alarms);
                
                if (n < MAX_FLIGHT_ALARMS - 1) {
                    memcpy(&fa[n], &fa[n+1], sizeof(struct flight_alarm) * (nr_alarms - n - 1));
                    memcpy(&fac[n], &fac[n+1], sizeof(struct flight_alarm) * (nr_alarms - n - 1));
                }
                for (t = n; t < nr_alarms - 1; t++)
                    fa[t].cfg--;
            ...
            case 2:
                ...
                if (n == 0xff) {
                    ...
                    if (n == nr_alarms) {
                        /* add new */
                        if (n == MAX_FLIGHT_ALARMS) {
                            shell_printf("Max nr of alarms reached!\n");
                            return;
                        }
                        shell_printf("Adding new alarm id=%u\n", id);

                        fac = &config.flight_alarm[n];
                        fac->props.id = id;
                        fac->timer = 0;
                        fac->value = 0;
                        fa = &stats.fl_alarms[n];
